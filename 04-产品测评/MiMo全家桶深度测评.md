...
> [!NOTE]
> 一句话总结：一年时间，小米靠 `MOPD` 把训练成本砍到 1/50、`Hybrid Attention` 把推理成本砍到对手的 2.5%、`MIT` 协议把生态门槛砍到零，用"够用 + 便宜 + 能干活"的工程护城河，把 MiMo 全家桶从"性价比"打进了全球开源第一梯队。

## 目录

- [前言](#前言)
- [一、MiMo 家族全景图：一张完整作战地图](#一mimo-家族全景图一张完整作战地图)
- [二、端侧奠基，MiMo-7B 与 MiMo-VL-7B](#二端侧奠基mimo-7b-与-mimo-vl-7b)
- [三、效率先锋：MiMo-V2-Flash](#三效率先锋mimo-v2-flash)
- [四、旗舰推理基座：MiMo-V2-Pro](#四旗舰推理基座mimo-v2-pro)
- [五、全模态基座：MiMo-V2-Omni](#五全模态基座mimo-v2-omni)
- [六、语音合成：MiMo-V2-TTS](#六语音合成mimo-v2-tts)
- [七、旗舰再进化：MiMo-V2.5-Pro](#七旗舰再进化mimo-v25-pro)
- [八、全模态 Agent 进化，MiMo-V2.5](#八全模态-agent-进化mimo-v25)
- [九、语音双雄进化：MiMo-V2.5-TTS 与 V2.5-ASR](#九语音双雄进化mimo-v25-tts-与-v25-asr)
- [十、全家桶横向对比表](#十全家桶横向对比表)
- [十一、经济模型与商业可行性](#十一经济模型与商业可行性)
- [十二、开源策略与生态布局](#十二开源策略与生态布局)
- [十三、端云协同：人车家全生态的真正闭环](#十三端云协同人车家全生态的真正闭环)
- [十四、不擅长的事，客观短板](#十四不擅长的事客观短板)
- [十五、给不同角色的实操结论](#十五给不同角色的实操结论)
...
一年里，小米从一个"刚迈入大模型赛道的硬件公司"，被推到了 `Artificial Analysis` 全球开源综合智能榜单的**第七位**、**Agent 指数开源第一**，并在 `Text Arena` 国际盲测中拿下全球第五、`LabRank` 第四。
...
每一款都会从**架构、性能、成本、适用场景**四个维度展开，最后给出一张完整的横向对比表。
...
这件事在当时被低估了——一款 7B 参数的端侧模型，在数学和代码这两个最考验"硬推理"的赛道上跑赢了 32B 参数级的开源旗舰，意味着**小米从一开始走的就是"用工程优化抵消参数差距"的路线**。

它的核心价值在于**能耗仅为竞品的 1/5**，被深度集成进 `HyperOS 3.0`，成为小米手机本地化 AI 推理的引擎。从用户体感上讲，离线翻译、摘要、屏幕内容理解这些"端上即时反馈"的能力，几乎都靠它完成。
...
2026 年 3 月 19 日凌晨发布的 `MiMo-V2-Pro`，是小米第一次真正进入"旗舰"赛道。总参数超过 1T，激活参数 42B，采用创新的混合注意力架构，原生支持 `1M` 超长上下文长度。在全球权威大模型综合智能排行榜 `Artificial Analysis` 上，`MiMo-V2-Pro` 位列**全球第八、国内第二**。
...
`V2-Pro` 的最佳战场是**长难任务**：完整的代码工程、跨多个工具的复杂业务流、需要数小时连续推理的研究型任务。它不是用来做客服对话的，而是用来"代替一个初级工程师工作半天"的。

## 五、全模态基座：MiMo-V2-Omni

### 5.1 原生统一架构，不是后接的多模态

`MiMo-V2-Omni` 在 3 月 19 日同期发布，定位是面向 Agent 时代的**全模态基座**。它的关键不在于"支持多模态"，而在于"原生统一处理"——大多数多模态模型是各模态分别处理后再拼接，而 `V2-Omni` 把图像、视频、音频、文本作为同一学习框架中的一等公民统一表示。
...
它最有意思的能力是**同一句话内完成语气转折和情感递变**——这是绝大多数 `TTS` 模型做不到的。它还能在唱歌时准确表达音高和节奏，少样本声音克隆（3 句样本）就能适配口音、语气甚至方言。
...
`V2.5-Pro` 这一代最大的态度转变，是把 `SWE-Bench Pro` 这种**抗污染测试集**放到了宣传榜首位。
...
> [!TIP]
> 这些 demo 的共同点是**长程、多工具、可交付**——它们想证明的不是"模型聪明"，而是"模型能把活干完"。
...
它的两个关键升级：**第一**，在 `Claw-Eval` 等权威 Agent 评测中超过 `V2-Pro` 水平，胜任日常简单任务，`API` 成本降低约 50%；**第二**，在 `VideoMME`、`CharXiv`、`MMMU-Pro` 等评测中跨模态推理、视频理解、图表分析等能力提升，逼近甚至超越业界顶级闭源模型。
...
`V2.5-TTS` 在 `V2-TTS` 的基础上进一步强化了情感连续性和多语种、多方言支持。而 `V2.5-ASR` 是这一代新增的**语音识别专项模型**，对车机、家居、AI 眼镜等远场、噪声、方言场景做了重点优化。
...
- **夜间折扣**：北京时间每天 00:00–08:00，所有模型 Credits 消耗速率再打 8 折
- **订阅优惠**：新增"连续包月"和"包年"订阅，老用户开通自动续费享次月 7 折，包年享全年 88 折
- **额度重置**：所有已购 `Token Plan` 用户截至 4 月 22 日 22:00 前的 Credits 全量重置

### 11.2 没有 5 小时窗口限制

绝大多数 Coding Plan 都有"每 5 小时 N 次请求"的硬限制，灵感来了额度没了是常态。

> [!TIP]
> `MiMo Token Plan` 用统一 `Credits` 体系，6000 万 Credits 想一次花完就一次花完，不存在窗口期。这对独立开发者和高强度 vibe coding 用户而言是个被低估的卖点。
...
`V2-Flash` 与 4 月 28 日开源的 `V2.5-Pro` 全部采用最宽松的 `MIT` 协议，模型权重和推理代码均可自由商业使用甚至闭源再分发。

> [!IMPORTANT]
> 这比 `Apache 2.0` 还宽松一档，对企业法务而言意义重大。
...
`WPS 灵犀`接入 `V2-Pro` 后，原生支持 Word、Excel、PPT、PDF 四大主流格式，覆盖超 95% 的日常文档类型，这是国产大模型在办公场景的最深度集成之一。
